...
VertexShaderOutput VS_BaseLightingWithFog(VertexShaderInput input)
{
...
float4 PS_BaseLightingWithFog(VertexShaderOutput input) : COLOR0
{
	...
	// view distance for fog effect
	float viewDistance = length(input.ViewDirection);
	// fog effect
	float fog = clamp((viewDistance - FogStart) / (FogEnd - FogStart), 0.0f, 1.0f);

	// returning to the frame buffer
	return float4(lerp(output, FogColor, fog), 1.0f);
}



technique BaseLightingWithFogTechnique
{
	pass Pass0
	{
		// TODO: set renderstates here.
		VertexShader = compile vs_2_0 VS_BaseLightingWithFog();
		PixelShader = compile ps_2_0 PS_BaseLightingWithFog();
	}
}
